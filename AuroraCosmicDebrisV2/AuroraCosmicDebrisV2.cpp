...
constexpr size_t kMaxPreDelaySamples = 24000;  // 0.5 s
...
// Efficient variable-width band-pass states: HP followed by LP.
...
// WARP pitch read state, only used by first two lines.
float pitch_phase[2] = {0.13f, 0.61f};

// Main six physical controls are stored so SHIFT editing cannot disturb them.
...
// Three secondary banks, edited relatively while SHIFT is held.
float shift_param[3][6] = {
    // PRE, RATE, WET GAIN, SIDECHAIN, SLEW, SPREAD
    {0.00f, 0.25f, 0.333f, 0.00f, 0.12f, 0.35f},
    // CENTER, WIDTH, SHAPE, ANOMALY, WARP FACTOR, CLOCK DIV TRIM
    {0.50f, 0.00f, 0.00f, 0.00f, 0.50f, 0.50f},
    // GATE MODE, SEND, SYNC, BLAST, GLITCH, CLOCK RANGE
...
bool prev_action_gate = false;
bool prev_both = false;
bool prev_shift_both = false;

uint64_t sample_counter = 0;
uint64_t last_ext_clock_sample = 0;
uint64_t ext_clock_period = 24000; // 120 BPM quarter note default
...
// UI state, written in audio callback and displayed in main loop.
...
// Fast sine-like curve, adequate for LFO modulation and much cheaper than sinf in 16 lines.
inline float FastSine(float phase)
{
    phase -= floorf(phase);
    float x = phase * kTwoPi - kPi;
...
void UpdateControls(bool shift)
{
    ...
    if(shift)
    {
        ...
        for(int i = 0; i < 6; ++i)
        {
            ...
            {
                shift_param[shift_bank][i]
                    = Clamp01(shift_param[shift_bank][i] + delta);
                shift_capture[i] = now;
            }
        }
    }
    else
    {
        if(shift_was_down)
        {
            // WMD-style soft takeover after leaving SHIFT.
            for(int i = 0; i < 6; ++i)
        ...
        for(int i = 0; i < 6; ++i)
        {
            ...
            if(!normal_pickup[i])
            {
                if(fabsf(raw - normal_param[i]) <= kPickupThreshold)
                    normal_pickup[i] = true;
            }
            if(normal_pickup[i])
                normal_param[i] = raw;
...
// In-place normalized Walsh-Hadamard transform: a computationally efficient
// dense 16x16 feedback matrix. 1/4 normalization keeps energy stable.
...
float LfoValue(int i, float shape)
{
    const float s = FastSine(lfo_phase[i]);
    const float t = Triangle(lfo_phase[i]);
    const float r = random_prev[i]
                  + (random_next[i] - random_prev[i]) * SmoothStep(lfo_phase[i]);
...
float PitchFeedbackRead(int i, float delay, float ratio, float phase_inc)
{
    ...
    // Two triangular crossfades, 180 degrees apart.
...
float FeedbackAmount(float knob)
{
    knob = Clamp01(knob);
    if(knob <= 0.5f)
        return knob * 2.0f; // exactly 100% at centre
    const float x = (knob - 0.5f) * 2.0f;
    return 1.0f + 0.28f * x * x; // controlled runaway zone
}
...
float BaseDelaySamples(float time_knob, bool sync_mode)
{
    if(sync_mode)
    {
        ...
        const float d = static_cast<float>(period)
                      * ClockDivision(time_knob, shift_param[2][5]);
        return fclamp(d, 19.2f, static_cast<float>(kMaxDelaySamples - 4096));
    }

    // 400 us to 1.5 s, exponential free-running range.
    const float seconds = 0.0004f * powf(3750.0f, Clamp01(time_knob));
    return fclamp(seconds * sample_rate, 19.2f,
...
void ComputeDelayTargets(float base, float spray, uint8_t m)
{
    ...
    for(int i = 0; i < 16; ++i)
    {
        ...
        if(m == 1)
        {
            // RATIO: at zero Spray, every other line is exactly twice as long.
            const float start = (i & 1) ? 2.0f : 1.0f;
            ratio = start + (ratio_full[i] - start) * spray;
        }
        else
        {
            // BLUR and WARP: first line stays at TIME; the others progressively shorten.
                   ...
                   size_t size)
{
    ...
    // Gate 1 is CLOCK, Gate 2 is the configurable action gate.
    const bool clock_gate = hw.GetGateState(GATE_FREEZE);
    if(clock_gate && !prev_clock_gate)
        RegisterClock();
    prev_clock_gate = clock_gate;

    const int gate_mode = static_cast<int>(shift_param[2][0] * 2.999f);
    const bool action_gate = hw.GetGateState(GATE_REVERSE);
    const bool gate_high = action_gate;
    prev_action_gate = action_gate;
    ...
    // Main controls plus their Aurora CV inputs.
    const float time = Clamp01(normal_param[0] - hw.GetCvValue(CV_TIME)); // WMD polarity: more CV = faster
    ...
    const float rate_hz = 0.025f * powf(320.0f, shift_param[0][1]); // 0.025..8 Hz
    ...
    // Filter width: at WIDTH=0 it becomes effectively full-band.
    const float center_hz = 80.0f * powf(200.0f, center); // 80 Hz .. 16 kHz
    ...
    // SLEW: near CCW uses click-free dual-tap crossfades; otherwise creates doppler glides.
    ...
    for(int i = 0; i < 16; ++i)
    {
        if(crossfade_mode)
        {
            if(fabsf(delay_target[i] - xfade_to[i]) > 6.0f)
            {
    ...
    // WARP pitch parameters are constant for the whole block.
    ...
    // Anomaly bit depth, calculated once per block.
    const int anomaly_bits = 16 - static_cast<int>(anomaly * 12.0f);
    const float anomaly_levels = static_cast<float>(1 << fclamp(anomaly_bits, 4, 16));

    const float pre_samples = fclamp(pre_delay_ms * 0.001f * sample_rate,
                                     1.0f,
                                     static_cast<float>(kMaxPreDelaySamples - 2));

    // Advance LFO phases per-sample with spread encoded as phase offsets at read time.
    const float lfo_inc = rate_hz / sample_rate;
    const float mod_depth = 0.28f * mod_amount * mod_amount;

    // Normal mix versus SEND behavior.
    ...
    for(size_t n = 0; n < size; ++n)
    {
        ...
        if(fabsf(in_r) < 0.00001f && fabsf(in_l) > 0.00005f)
            in_r = in_l; // approximate the original left-input normalization
        ...
        // Input envelope for SIDE CHAIN ducking.
        const float level = fminf(1.0f, fmaxf(fabsf(in_l), fabsf(in_r)) * 0.70f);
        if(level > input_env)
            input_env += 0.055f * (level - input_env);
        else
            input_env += 0.0012f * (level - input_env);
        ...
        for(int i = 0; i < 16; ++i)
        {
            // Each line has its own LFO phase. SPREAD pushes phases apart.
            float phase = lfo_phase[i] + spread * (static_cast<float>(i) / 16.0f);
            phase -= floorf(phase);
            const float lfo = LfoValue(i, shape);
            ...
            // FREEZE bypasses filtering, modulation and pitch processing per manual.
            filtered[i] = freeze ? raw[i]
                                 : BandPass(i, raw[i], hp_coeff, lp_coeff);
            matrix[i] = filtered[i];

            // Advance each base phase and refresh smooth-random targets on wrap.
            lfo_phase[i] += lfo_inc;
            if(lfo_phase[i] >= 1.0f)
            {
                lfo_phase[i] -= 1.0f;
        ...
        // Dense 16x16 feedback network. Scatter crossfades identity -> dense matrix.
        float dense[16];
        for(int i = 0; i < 16; ++i)
            dense[i] = matrix[i];
        Hadamard16(dense);
        ...
        for(int i = 0; i < 16; ++i)
        {
            // Odd-numbered original lines (zero-based even) sum left; even-numbered sum right.
            if((i & 1) == 0)
                wet_l += filtered[i] * 0.235f;
            else
                wet_r += filtered[i] * 0.235f;
        }

        // WARP input distribution: SPRAY 0 feeds only first two lines; by 50% all lines are equal.
        ...
        for(int i = 0; i < 16; ++i)
        {
            ...
            // ANOMALY: bit reduction plus progressively stronger short-buffer stutter.
            ...
            // WARP: pitch shifters are in the feedback loop of the first two lines only.
            ...
            {
                if(i < 2)
                    inject_weight = warp_all + warp_focus * 2.4f;
                else
                    inject_weight = warp_all;
            }

            float inject = freeze ? 0.0f
                                  : channel_source * send_level * inject_weight * 0.42f;
            if(blast)
                inject = FastSat(inject * blast_strength);

            const float write = FastSat(inject + fb * feedback);
            delay_line[i].Write(Bound(write, 2.2f));
        }

        // KILL fades the wet path before main-loop reset clears all buffers.
...
void UpdateLeds()
{
    ...
    if(!ui_shift_down)
    {
        // TIME blue, FEEDBACK red, MIX yellow, SPRAY green, SCATTER cyan, MOD magenta.
        ...
        // Red flashing pickup warning, matching the manual's soft-takeover concept.
    ...
    {
        // Bank A = amber, B = cyan, C = white. Brightness shows parameter value.
        for(int i = 0; i < 6; ++i)
        {
    ...
    // Bottom three LEDs show the processing mode.
    hw.SetLed(LED_BOT_1, ui_mode == 0 ? 0.0f : 0.03f,
                          ui_mode == 0 ? 0.25f : 0.03f,
                          ui_mode == 0 ? 1.0f : 0.03f); // BLUR blue
    hw.SetLed(LED_BOT_2, ui_mode == 1 ? 0.05f : 0.03f,
                          ui_mode == 1 ? 1.0f : 0.03f,
                          ui_mode == 1 ? 0.18f : 0.03f); // RATIO green
    hw.SetLed(LED_BOT_3, ui_mode == 2 ? 1.0f : 0.03f,
                          0.0f,
                          ui_mode == 2 ? 1.0f : 0.03f); // WARP magenta

    if(ui_blast)
        hw.SetLed(LED_FREEZE, 1.0f, 0.22f, 0.0f);
    else if(ui_freeze)
        hw.SetLed(LED_FREEZE, 1.0f, 1.0f, 1.0f);
    else if(ui_send)
        hw.SetLed(LED_FREEZE, 0.55f, 0.0f, 0.0f);
    else
        hw.SetLed(LED_FREEZE, 0.02f, 0.02f, 0.02f);
...
int main(void)
{
    ...
    // Put an unmistakable valid LED state on the hardware before audio begins.
    hw.ClearLeds();
    ...
    // Important: use Aurora's default block size, matching the hardware-confirmed EchoGarden runtime.
    hw.StartAudio(AudioCallback);
